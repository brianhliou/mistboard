...
export const CO_UP_VI: Record<string, string> = {
  ...
  "PikaJieQi is a fork of [Pikafish](https://github.com/official-pikafish/Pikafish), the open-source xiangqi engine, on its jieqi branch. Classical alpha-beta search with a hand-written evaluation and no neural network. What makes it a jieqi engine rather than a xiangqi one is that it treats every face-down piece as a chance node, scoring a move as the average over each piece that tile could still be. It only ever sees the face-down board, and a test fails the build if a hidden identity ever leaks into what it is sent. [Pikafish itself](/blog/pikafish), at full strength for standard xiangqi, is on the site too.": "PikaJieQi là bản fork của [Pikafish](https://github.com/official-pikafish/Pikafish), engine cờ tướng mã nguồn mở, trên nhánh dành cho cờ úp. Tìm kiếm alpha-beta cổ điển với hàm đánh giá viết tay, không có mạng nơ-ron. Điều làm nó thành engine cờ úp chứ không phải engine cờ tướng là nó coi mỗi quân úp như một nút may rủi, chấm điểm một nước bằng trung bình trên từng quân mà ô đó còn có thể là. Nó chỉ nhìn thấy bàn cờ đã úp, và có một bài kiểm thử làm hỏng bản dựng nếu một thân phận bị lộ ra trong dữ liệu gửi cho nó. [Chính Pikafish](/blog/pikafish), ở sức mạnh tối đa cho cờ tướng thường, cũng có trên trang.",

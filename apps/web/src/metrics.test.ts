...
const adminMetrics = {
  ...
  // Includes a retired id (luzhanqi) that the live-shelf filter drops.
  humanGamesByVariant: { xiangqi: 402, 'dark-xiangqi': 180, luzhanqi: 60 },
...
describe('metrics page', () => {
  ...
  it('admin /metrics adds player cards, weekly charts, the result split, and an engines block', async () => {
    ...
    expect(variantLabels).toEqual(['Xiangqi', 'Fog Xiangqi']);
    expect(variantLabels).not.toContain('Luzhanqi');
  });

...
const { values } = parseArgs({
  options: { dir: { type: 'string' }, slug: { type: 'string' }, readme: { type: 'boolean', default: false } },
});
...
// completeness
for (const r of raw) if (!recs[String(r.id)]) problems.push(`record ${r.id} ${r.title} missing from edits`);
for (const id of Object.keys(recs)) if (!rawById.has(id)) problems.push(`edits record ${id} is not in the mined data`);

// per record
const SIMPLIFIED_ONLY = /[马车龙凤将来对开关东门长时间说话当会电点边区]/;
const EDITORIAL = /(19\d\d年|20\d\d年|诠订|詮訂|内容提要|內容提要|编者|編者|http|www\.|本局即|杨官璘|楊官璘|屠景明|裘望禹)/;
const EM = /—/;
const byVol = {};
const enMap = new Map();
...
for (const [id, r] of Object.entries(recs)) {
  ...
  const tag = `${id} ${r.zh ?? '?'}`;
  for (const f of ['n', 'zh', 'hant', 'en', 'sound', 'publish']) if (r[f] === undefined) problems.push(`${tag}: missing ${f}`);
  if (r.publish) {
    publish += 1;
    (byVol[vol] ??= []).push(r.n);
    ...
    if (r.en && r.en.split(/\s+/).length > 9) warnings.push(`${tag}: English title has ${r.en.split(/\s+/).length} words`);
    if (r.en) {
      const prev = enMap.get(r.en);
      if (prev && prev.zh !== r.zh) problems.push(`${tag}: English "${r.en}" also used by ${prev.id} ${prev.zh}`);
      enMap.set(r.en, { id, zh: r.zh });
    }
    if (r.hant === r.zh && SIMPLIFIED_ONLY.test(r.zh)) warnings.push(`${tag}: hant equals zh but zh has a Simplified-only character`);
  }
  for (const f of ['en', 'gloss', 'notes', 'soundNote']) if (typeof r[f] === 'string' && EM.test(r[f])) problems.push(`${tag}: em dash in ${f}`);
  for (const [k, t] of Object.entries(r.prose ?? {})) {
    if (EM.test(t)) problems.push(`${tag}: em dash in prose[${k}]`);
    if (EDITORIAL.test(t)) problems.push(`${tag}: editorial marker in kept prose[${k}]: ${t.slice(0, 30)}`);
    const src = rawById.get(id)?.comments?.[k];
    if (src === undefined) problems.push(`${tag}: prose[${k}] has no source comment`);
    else if (!src.includes(t.replace(/, /g, '—')) && !src.includes(t)) warnings.push(`${tag}: prose[${k}] is not verbatim from the record`);
  }
}
...
const volumes = Array.isArray(book.volumes) && book.volumes.length ? book.volumes : [{ vol: 1, zh: book.zh }];
...
for (const v of Object.keys(byVol).map(Number)) {
  ...
  for (const loc of ['en', 'zh-Hans', 'zh-Hant']) {
    if (!st[loc]?.name || !st[loc]?.description) problems.push(`vol ${v}: study ${loc} name/description missing`);
    for (const f of ['name', 'description']) if (EM.test(st[loc]?.[f] ?? '')) problems.push(`vol ${v}: em dash in study ${loc} ${f}`);
  }
  ...
  if (/[㐀-鿿]/.test(st.en?.name ?? '')) warnings.push(`vol ${v}: English study name contains Han script`);
}
...
const sample = pubs.filter((_, i) => i % Math.max(1, Math.floor(pubs.length / Math.max(1, Math.round(pubs.length / 10)))) === 0).slice(0, 12);

console.log(`${slug}: ${Object.keys(recs).length} records, ${publish} publish; ${problems.length} problem(s), ${warnings.length} warning(s)`);
...
if (values.readme) {
  ...
  if (existsSync(path)) {
    ...
    const held = Object.entries(recs)
      ...
      .map(([id, r]) => `| ${r.n ?? ''} | ${id} | ${r.zh} | ${errById.get(id) ? 'replay error' : r.sound ?? ''} | ${(errById.get(id) ?? r.soundNote ?? r.notes ?? '').replace(/\|/g, '/').slice(0, 220)} |`);
    const lines = [
      ...
      '',
      `Study: ${volumes.map((v, i) => studyFor(i + 1)?.en?.name ?? '(no study copy)').join(' / ')}`,
      '',
      '## Held back',
      '',
      '| n | id | title | why | note |',
      ...
      '',
      ...sample.map(([id, r]) => `- ${id} ${r.zh} / ${r.hant} / "${r.en}"${r.gloss ? ` (${r.gloss.slice(0, 120)})` : ''}`),
      '',
      '## Checks',
      '',

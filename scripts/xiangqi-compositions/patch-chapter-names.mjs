...
const { values } = parseArgs({
  options: { edits: { type: 'string' }, data: { type: 'string' }, slugs: { type: 'string' }, apply: { type: 'boolean', default: false } },
});
if (!values.edits || !values.data || !values.slugs || !process.env.DATABASE_URL) {
  console.error('usage: DATABASE_URL=... patch-chapter-names.mjs --edits <file> --data <file> --slugs a,b [--apply]');
  process.exit(2);
}
...
for (const [id, r] of Object.entries(edits.records ?? {})) {
  if (r && typeof r === 'object' && r.publish) want.set(`${volById.get(id) ?? 1}:${r.n}`, `${r.n}. ${r.en}`);
}
...
try {
  for (const [vi, slug] of values.slugs.split(',').filter(Boolean).entries()) {
    ...
    const chapters = await client.query('SELECT id, name FROM study_chapters WHERE study_id = $1', [study.rows[0].id]);
    for (const c of chapters.rows) {
      ...
      changed += 1;
      if (values.apply) await client.query('UPDATE study_chapters SET name = $2, updated_at = now() WHERE id = $1', [c.id, next]);
    }
  }
} finally {
  await client.end();
}
console.log(`${changed} chapter name(s) ${values.apply ? 'updated' : 'would change (pass --apply)'}`);

  ...
  // Regenerate, never hand-edit.
  {
    ...
    const boardLiteral = (board: XiangqiBoard) =>
      `{ ${Object.entries(board)
        ...
        .join(', ')} }`;
    // The article's playthrough: eight frames of the forward36 veteran game,
    // each a kernel-replayed position with the move that reached it.
    const FRAMES: Array<{ ply: number; narrative: string }> = [
      {
        ply: 0,
        narrative:
      ...
      { ply: 431, narrative: 'Move 216. The second chariot falls. The general is bare.' },
      {
        ply: 435,
        narrative:
    ...
    for (const f of FRAMES)
      if (f.ply > GW.moves.length) throw new Error(`frame ply ${f.ply} past the end`);
    const frameLiteral = (f: { ply: number; narrative: string }) => {
      const st = replay(GW, f.ply);
      const last = f.ply ? GW.moves[f.ply - 1]! : null;
      const arrow = last
        ? `arrows: [{ from: '${last.slice(0, last.length - (last.endsWith('10') ? 3 : 2))}' as XiangqiSquare, to: '${last.slice(last.length - (last.endsWith('10') ? 3 : 2))}' as XiangqiSquare }],`
        : '';
      return `  {
    svg: () =>
      xqSvg(
        ...
        FIGURE_H,
        xqBoardSvg({
          state: xqVisionDemoState('horde-game-${f.ply}', ${boardLiteral(st.board)}),
          x: PAIR_GAP_X / 2,
          y: 0,
          label: 'PLY ${f.ply}${f.ply === GW.moves.length ? ': NO MOVE, NO CHECK, RED WINS' : ''}',
          perspective: 'red',
          ${arrow}
          veteranSoldiers: true,
        }),
      ),
    narrative: '${f.narrative.replace(/'/g, "\\'")}',
  },`;
    };
    const module = `// Generated by scripts/gen-horde-diagrams.mts in this repository (--blog).
...
export const HORDE_BEFORE_DROP: Board = ${boardLiteral(before.board)};
/** Fifteen moves later: ${eaten} soldiers gone from behind. */
export const HORDE_AFTER_DROP: Board = ${boardLiteral(later.board)};
